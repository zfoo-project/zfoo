...
public class EnhanceMapSerializer implements IEnhanceSerializer {
    ...
    public void writeObject(StringBuilder builder, String objectStr, Field field, IFieldRegistration fieldRegistration) {
        if (CutDownMapSerializer.getInstance().writeObject(builder, objectStr, field, fieldRegistration, CodeLanguage.Enhance)) {
            return;
        }
    ...
    public String readObject(StringBuilder builder, Field field, IFieldRegistration fieldRegistration) {
        var cutDown = CutDownMapSerializer.getInstance().readObject(builder, field, fieldRegistration, CodeLanguage.Enhance);
        if (cutDown != null) {
            return cutDown;
        }
        ...
        builder.append(StringUtils.format("Map {} = CollectionUtils.erasureMap({});", map, size));
